...
target = (20,22,0) 
bot = car_bot((8,4),np.deg2rad(90),scale)
random.seed(128)
...
# path_success,path = bot.find_path_to_goal(target,obstacle_grid)
# if not path_success:
...
plt.savefig("car_setup.png")

ax.cla()

fig, ax = plt.subplots()
test_vels = [1.5,1,0.5,-1]
test_angs = [0,-35,45,10]
test_times = [200,200,100,300]
frame_i = 0
for i in range(3):
    vel = test_vels[i]
    ang = test_angs[i]
    ...
    for t in range(time):
        ...
        ax.scatter(bot.x*scale,bot.y*scale, color='red',s=20)  # bots center of rotation
        ax.scatter(target[1]*scale,target[0]*scale, color='k',s=20) # goal point
        plt.savefig(f"map_frames/car_frame_{frame_i}")
        frame_i += 1
        # break
generate_animation()
